...
[**TERMINOLOGÍA	1**](#terminología)

[**1\. CONEXIÓN	4**](#1.-conexión)

[1.1 Transporte	4](#1.1-transporte)

[1.2 Puertos	4](#1.2-puertos)

[1.3 Descubrimiento de servidores	4](#1.3-descubrimiento-de-servidores)

[**2\. EL IDIOMA DE LOS MENSAJES	6**](#2.-el-idioma-de-los-mensajes)

[2.1 Delimitación de mensajes (framing)	6](#2.1-delimitación-de-mensajes-\(framing\))

[2.2 Formato y codificación	6](#2.2-formato-y-codificación)

[2.3 Catálogo de mensajes	7](#2.3-catálogo-de-mensajes)

[*2.3.1 Vista general del catálogo	7*](#2.3.1-vista-general-del-catálogo)

[*2.3.2 Detalle de campos por mensaje	8*](#2.3.2-detalle-de-campos-por-mensaje)

[*2.3.3 Constantes y límites acordados	10*](#2.3.3-constantes-y-límites-acordados)

[**3\. REGLAS DE LA PARTIDA	12**](#3.-reglas-de-la-partida)

[3.1 Secuencia de la partida	12](#3.1-secuencia-de-la-partida)

[3.2 Sistema de coordenadas	12](#3.2-sistema-de-coordenadas)

[3.3 Reglas del dominio (normativas)	12](#3.3-reglas-del-dominio-\(normativas\))

[**4\. AUTORIDAD Y SINCRONIZACIÓN	14**](#4.-autoridad-y-sincronización)

[4.1 Autoridad y validaciones	14](#4.1-autoridad-y-validaciones)

[4.2 Sincronización del estado	15](#4.2-sincronización-del-estado)

[**5\. MANEJO DE FALLAS Y DESCONEXIONES	16**](#5.-manejo-de-fallas-y-desconexiones)

[5.1 Manejo de errores	16](#5.1-manejo-de-errores)

[*5.1.1 Tabla de errores comunes	16*](#5.1.1-tabla-de-errores-comunes)

[5.2 Desconexiones	17](#5.2-desconexiones)

[*5.2.1Casos especiales de desconexión	17*](#5.2.1casos-especiales-de-desconexión)

[5.3 Decisiones de empate y propiedades del dominio	18](#5.3-decisiones-de-empate-y-propiedades-del-dominio)

[*5.3.1 Propiedades que deben permanecer verdaderas	18*](#5.3.1-propiedades-que-deben-permanecer-verdaderas)

[**6\. CONSIDERACIONES DE SEGURIDAD	20**](#6.-consideraciones-de-seguridad)

[6.1 Ausencia de Cifrado y Autenticación	20](#6.1-ausencia-de-cifrado-y-autenticación)

[6.2 Denegación de Servicio (DoS) y Control de Memoria	20](#6.2-denegación-de-servicio-\(dos\)-y-control-de-memoria)

[6.3 Validación de Entradas e Inyección en el Parser JSON	20](#6.3-validación-de-entradas-e-inyección-en-el-parser-json)

[6.4 Integridad de la Partida y Prevención de Trampas	20](#6.4-integridad-de-la-partida-y-prevención-de-trampas)

[**7\. CONTROL DE CAMBIOS	22**](#7.-control-de-cambios)

[7.1 Versionado	22](#7.1-versionado)

[7.2 Procedimiento para modificar el estándar	22](#7.2-procedimiento-para-modificar-el-estándar)

[7.3 Historial de cambios	22](#7.3-historial-de-cambios)
...
### **1.1 Transporte** {#1.1-transporte}

Define la tecnología encargada de mover los bytes entre las máquinas participantes y las garantías de entrega que esta ofrece. Todos los proyectos de la clase deben acordar el mismo transporte para que sus conexiones sean compatibles entre sí.Se establece un esquema híbrido:
...
### **1.2 Puertos** {#1.2-puertos}

Establece los números de puerto en los que los servidores escuchan, tanto para el descubrimiento como para la partida. Separar ambos puertos evita que la búsqueda de servidores interfiera con el tráfico del juego en curso.Se definen dos tipos de puerto:
...
### **1.3 Descubrimiento de servidores** {#1.3-descubrimiento-de-servidores}

Describe el procedimiento mediante el cual un cliente localiza los servidores disponibles en la red local sin conocer sus direcciones IP de antemano. Este mecanismo depende del puerto UDP definido en la sección 1.2.El descubrimiento sigue dos vías:
...
## **2\. EL IDIOMA DE LOS MENSAJES** {#2.-el-idioma-de-los-mensajes}

**Diagrama de Framing:**

`[JSON Message 1]\n[JSON Message 2]\n[Part of JSON 3...]`

### **2.1 Delimitación de mensajes (framing)** {#2.1-delimitación-de-mensajes-(framing)}

Especifica la regla que permite identificar dónde termina un mensaje y dónde comienza el siguiente. Este punto es indispensable porque TCP entrega los datos como un chorro continuo de bytes, sin ninguna separación natural entre mensajes: dos mensajes pueden llegar pegados en una sola lectura, o uno solo puede llegar partido en dos lecturas distintas.Se define la siguiente regla:
...
### **2.2 Formato y codificación** {#2.2-formato-y-codificación}
...
### **2.3 Catálogo de mensajes** {#2.3-catálogo-de-mensajes}

#### **2.3.1 Vista general del catálogo** {#2.3.1-vista-general-del-catálogo}

| Tipo | Dirección | Fase | Propósito | Campos principales |
| :---- | :---- | :---- | :---- | :---- |
| discover | C → UDP | Cualquiera | Buscar servidores en la red | v |
| server\_info | S → UDP | Cualquiera | Anunciar el servidor encontrado | v, name, tcp\_port, state, players |
...
| welcome | S → C | Lobby | Asignar identidad y constantes | player\_id, config |
...
| game\_over | S → C | Playing-Finished | Anunciar al ganador | winner |
| error | S → C | Cualquiera | Rechazar una acción inválida | reason |

#### 

#### **2.3.2 Detalle de campos por mensaje** {#2.3.2-detalle-de-campos-por-mensaje}
...
* **server\_info** (Servidor → Unicast UDP | Fase: cualquiera):  
...
#### **2.3.3 Constantes y límites acordados** {#2.3.3-constantes-y-límites-acordados}

| Categoría | Constante / Límite | Valor | Significado |
| :---- | :---- | :---- | :---- |
| Constantes en welcome.config | map\_size | 1000 | El mapa mide 1000 x 1000 unidades lógicas. |
|  | circle\_radius | 300 | El círculo central mide 300 unidades de radio. |
|  | player\_radius | 15 | El cuerpo del jugador mide 15 unidades de radio. |
|  | interact\_radius | 40 | Hasta 40 unidades de distancia para tomar o robar la bandera. |
|  | speed | 200 | 200 unidades por segundo de velocidad de movimiento. |
|  | tick\_rate | 20 | 20 envíos de estado por segundo. |
| Constantes del Servidor | countdown\_seconds | 5 | La cuenta regresiva dura 5 segundos antes de iniciar. |
|  | min\_players | 2 | Mínimo de jugadores para disparar y mantener el countdown. |
|  | post\_game\_seconds | 5 | Pausa tras game\_over antes del regreso al lobby. |
|  | circle\_center | (500, 500\) | Centro del mapa y del círculo (= map\_size / 2). |
|  | spawn\_radius\_min / max | 350 / 450 | Anillo de aparición alrededor del centro (sección 3.3). |
|  | victory\_distance | 315 | circle\_radius \+ player\_radius; distancia al centro que se debe superar para ganar (sección 3.3). |
| Límites del Protocolo | max\_players | 100 | Máximo de jugadores por partida (según el enunciado). |
|  | name\_max\_length | 20 | Largo máximo del nombre del jugador en caracteres. |
|  | message\_max\_size | 64 KB | Tamaño máximo de un mensaje individual. |
...
## **3\. REGLAS DE LA PARTIDA** {#3.-reglas-de-la-partida}

**Ciclo de Vida:**

`Lobby -> Countdown (5s) -> Playing -> Game Over -> Pausa (5s) -> Lobby`

### **3.1 Secuencia de la partida** {#3.1-secuencia-de-la-partida}
...
### **3.2 Sistema de coordenadas** {#3.2-sistema-de-coordenadas}
...
### **3.3 Reglas del dominio (normativas)** {#3.3-reglas-del-dominio-(normativas)}
...
## **4\. AUTORIDAD Y SINCRONIZACIÓN** {#4.-autoridad-y-sincronización}

### **4.1 Autoridad y validaciones** {#4.1-autoridad-y-validaciones}
...
### **4.2 Sincronización del estado** {#4.2-sincronización-del-estado}
...
## **5\. MANEJO DE FALLAS Y DESCONEXIONES** {#5.-manejo-de-fallas-y-desconexiones}

### **5.1 Manejo de errores** {#5.1-manejo-de-errores}
...
#### **5.1.1 Tabla de errores comunes** {#5.1.1-tabla-de-errores-comunes}
...
| :---- | :---- | :---- |
| INVALID\_JSON | El texto recibido no es un JSON válido. | No, excepto si ocurre repetidamente. |
| UNKNOWN\_TYPE | El campo type contiene un mensaje desconocido. | No. |
| MISSING\_FIELD | Falta un campo obligatorio. | No. |
| INVALID\_FIELD | Un campo tiene un valor o tipo incorrecto. | No. |
| INVALID\_PHASE | La acción no está permitida en la fase actual. | No. |
| VERSION\_MISMATCH | Cliente y servidor usan versiones incompatibles. | Sí. |
| LOBBY\_FULL | El servidor alcanzó el máximo de jugadores. | Sí. |
| NAME\_INVALID | El nombre está vacío, es muy largo o no es válido. | No. |
| GAME\_STARTED | Se recibió join con el servidor en countdown o playing. | Sí, después de enviar el error. |
| MESSAGE\_TOO\_LARGE | El mensaje supera el tamaño máximo permitido. | Sí. |
| NOT\_JOINED | El cliente intenta jugar antes de enviar join. | No. |

### **5.2 Desconexiones** {#5.2-desconexiones}

| Situación | Acción del servidor | Estado de la bandera |
| :---- | :---- | :---- |
| **Desconexión en lobby** | Eliminar jugador y actualizar lobby. | Sin cambios. |
| **Desconexión en countdown** | Eliminar jugador y comprobar min\_players (2): si quedan menos de 2 jugadores, se aborta el countdown de inmediato y se envía en broadcast un mensaje lobby (regreso a la sala de espera). | Sin cambios. |
...
#### **5.2.1Casos especiales de desconexión** {#5.2.1casos-especiales-de-desconexión}
...
### **5.3 Decisiones de empate y propiedades del dominio** {#5.3-decisiones-de-empate-y-propiedades-del-dominio}
...
#### **5.3.1 Propiedades que deben permanecer verdaderas** {#5.3.1-propiedades-que-deben-permanecer-verdaderas}

* En cualquier `state` existe como máximo un `flag.owner` no nulo.  
* La bandera siempre tiene exactamente una ubicación: en el centro (500,500), en el punto donde quedó libre, o en la posición del portador.  
* El ganador se escribe una sola vez por partida: un solo `game_over` por ronda.  
* Cada mensaje recibido se procesa exactamente una vez y en orden de llegada; ningún mensaje muta el estado dos veces.  
* Un cliente no puede ganar enviando coordenadas ni declarando victoria: las posiciones y la victoria las calcula únicamente el servidor.  
* La misma secuencia de mensajes, en el mismo orden de llegada, produce el mismo resultado de dominio (determinismo).

## 

## **6\. CONSIDERACIONES DE SEGURIDAD** {#6.-consideraciones-de-seguridad}

### **6.1 Ausencia de Cifrado y Autenticación** {#6.1-ausencia-de-cifrado-y-autenticación}
...
### **6.2 Denegación de Servicio (DoS) y Control de Memoria** {#6.2-denegación-de-servicio-(dos)-y-control-de-memoria}
...
* **Límite de Tamaño de Mensaje (`message_max_size`):** Todo mensaje entrante que supere los 64 KB se rechaza con `MESSAGE_TOO_LARGE` y provoca el cierre inmediato del socket TCP. En UDP, los paquetes mayores a 64 KB se descartan silenciosamente.  
1. **Saneamiento de Nombres (`name_max_length`):** Los nombres de jugadores están restringidos a un máximo de 20 caracteres UTF-8 tras aplicar recorte de espacios (trim) y eliminación de caracteres de control o saltos de línea.  
* **Control de Conexiones (`max_players`):** El servidor debe limitar la sala a 100 conexiones simultáneas. Las solicitudes `join` excedentes se rechazan con `LOBBY_FULL` y el socket se cierra.

### **6.3 Validación de Entradas e Inyección en el Parser JSON** {#6.3-validación-de-entradas-e-inyección-en-el-parser-json}
...
### **6.4 Integridad de la Partida y Prevención de Trampas** {#6.4-integridad-de-la-partida-y-prevención-de-trampas}
...
### **7.2 Procedimiento para modificar el estándar** {#7.2-procedimiento-para-modificar-el-estándar}
...
### **7.3 Historial de cambios** {#7.3-historial-de-cambios}

| Versión | Fecha | Cambios | Autor(es) |
| :---- | :---- | :---- | :---- |
| 1.0.0 | 2026-07-18 | Versión inicial borrador. Contenía campos no definidos en el catálogo (flag\_version, action\_id, input\_seq, mode, etc.), timeout de 8 s, códigos de error inconsistentes, flag.owner con valores ambiguos (null o 0), reglas de victoria y descubrimiento incompletas. Usado como base para la revisión grupal. **No implementar.** | Clase CC8 2026 |
| 1.1.0 | 2026-07-25 | Versión consolidada. Cambios con respecto a 1.0.0: (1) eliminación de campos fantasma — todo desempate se basa en orden de llegada TCP \+ ejecución secuencial del servidor; (2) eliminación del timeout de 8 s (v1 no define timeout); (3) catálogo de errores unificado, con GAME\_STARTED como nuevo código; (4) flag.owner libre fijado exclusivamente en null; (5) min\_players=2 y aborto de countdown; (6) transición post-partida de 5 s con regreso al lobby; (7) rechazo de joins en countdown/playing; (8) desconexiones inferidas; (9) descubrimiento por broadcast dual; (10) SO\_REUSEADDR/SO\_REUSEPORT obligatorio; (11) centro del mapa fijado en (500,500) y victoria formalizada; (12) robo por distancia como único requisito; (13) bandera pegada al portador; (14) fórmula de spawn en anillo; (15) constantes de welcome.config declaradas fijas; (16) advertencia sobre \\r\\n; (17) límite de 64 KB; (18) lectura tolerante; (19) nueva sección 6\. **Incremento MENOR (1.1.0):** el campo v se mantiene en 1\. | Andrés Tobar |
| 1.2.0 | 2026-07-25  | Estructuración bajo formato RFC. Inclusión de Abstract, Terminología, Diagramas ASCII y Consideraciones de Seguridad. | Andrés Tobar |
